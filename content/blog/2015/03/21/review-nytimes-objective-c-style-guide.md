...
[TIApplication sharedApplication].delegate;
```
...
if文の条件に合致する処理は常に改行をさせる。『リーダブルコード』などでもお馴染みのトピックであり、きわめて妥当。
...
Googleのスタイルガイドについて「古い」と指摘した通り、インスタンス変数ではなくプロパティを使うべきとしている点も含め、妥当。
...
ブロックコメントをドキュメンテーション生成用のものには使って良いというルールについて細くすると、Xcode 5からサポートされてJavadocのような構文のことを指していると思われる。
```objc
/**
