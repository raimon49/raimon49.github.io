...
記事の後半では、"Co-authored-by"で署名する事で期待できる効果についても考察や私見を述べる。
...
* コミットメッセージとして、同席していたボブとの共同作者であることを明記するため `Co-authored-by: Bob <bob@example.com>` を付与した。アリスは同席していなかったため、有効にしなかった。
...
* `git cimob` が実行された時、 `commit --template` オプションで作者情報の記述されたファイル `git-co-authors.txt` を指定する。
    * `cimob` は別に `cipair` でも構わないし、短いのが好みなら `cim` でも良い。自分の覚えやすいものを設定する。
...
署名部分は、必ずしもコメントアウト状態でなくても構わない。常にペアワークしていて必ず共同作者として扱うなら、最初から有効行として用意しておいた方が便利である。
...
かれこれ半年間ほど使ってみて、思った以上にチームへ良い影響を与えている実感があるため、広まって欲しいと思っている。
